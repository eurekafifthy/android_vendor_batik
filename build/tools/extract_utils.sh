...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$BATIK_ROOT"/vendor/batik/build/tools/smali/baksmali.jar
        export SMALIJAR="$BATIK_ROOT"/vendor/batik/build/tools/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$BATIK_ROOT"/vendor/batik/build/tools/"$HOST"/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$BATIK_ROOT"/vendor/batik/build/tools/"$HOST"/compact_dex_converter
    fi
