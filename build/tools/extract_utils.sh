...
function setup_vendor() {
    ...
    export BATIK_ROOT="$3"
    if [ ! -d "$BATIK_ROOT" ]; then
        echo "\$BATIK_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$BATIK_ROOT/$OUTDIR" ]; then
        mkdir -p "$BATIK_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$BATIK_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDBP="$BATIK_ROOT"/"$OUTDIR"/Android.bp
    export ANDROIDMK="$BATIK_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$BATIK_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function write_makefile_header() {
    ...
    if [[ $INITIAL_COPYRIGHT_YEAR =~ $NUM_REGEX ]] && [ $INITIAL_COPYRIGHT_YEAR -le $YEAR ]; then
        ...
        if [ $YEAR -eq 2017 ]; then
            printf "# Copyright (C) 2017 The BatikOS Project\n" >> $1
        elif [ $INITIAL_COPYRIGHT_YEAR -eq $YEAR ]; then
            printf "# Copyright (C) $YEAR The BatikOS Project\n" >> $1
        elif [ $INITIAL_COPYRIGHT_YEAR -le 2017 ]; then
            printf "# Copyright (C) 2017-$YEAR The BatikOS Project\n" >> $1
        else
            printf "# Copyright (C) $INITIAL_COPYRIGHT_YEAR-$YEAR The BatikOS Project\n" >> $1
        fi
    else
        printf "# Copyright (C) $YEAR The BatikOS Project\n" > $1
    fi
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$BATIK_ROOT"/prebuilts/tools-lineage/common/smali/baksmali.jar
        export SMALIJAR="$BATIK_ROOT"/prebuilts/tools-lineage/common/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$BATIK_ROOT"/prebuilts/tools-lineage/"${HOST,,}"-x86/bin/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$BATIK_ROOT"/prebuilts/tools-lineage/"${HOST,,}"-x86/bin/compact_dex_converter
    fi
    ...
    if [ ! -f "$BATIK_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$BATIK_TARGET" >/dev/null; then
...
function extract() {
    ...
    local OUTPUT_ROOT="$BATIK_ROOT"/"$OUTDIR"/proprietary
    ...
    if [ -f "$SRC" ] && [ "${SRC##*.}" == "zip" ]; then
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            # If OTA is block based, extract it.
                ...
                python "$BATIK_ROOT"/vendor/batik/build/tools/sdat2img.py "$DUMPDIR"/system.transfer.list "$DUMPDIR"/system.new.dat "$DUMPDIR"/system.img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$BATIK_ROOT"/"$OUTDIR"/radio
